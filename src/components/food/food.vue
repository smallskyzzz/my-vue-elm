          ...
          <div class="rating-wrapper">
            <ul v-show="food.ratings && food.ratings.length">
              <li v-show="needShow(rating.rateType,rating.text)" v-for="rating in food.ratings" class="rating-item border-1px">
                ...
                <div class="time">{{rating.rateTime | formatDate}}</div><!--使用filter-->
                <p class="text">
                  <span :class="{'icon-thumb_up':rating.rateType===0,'icon-thumb_down':rating.rateType===1}"></span><!--判断到底显示哪个图标，牛皮-->{{rating.text}}
                </p>
              </li>
            </ul>
...
<script>
  ...
  import {formatDate} from '../../common/js/date'//export function的带花括号，其他直接export default
